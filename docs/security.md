...
| `GET /api/medical-documents` | Requires authenticated `PATIENT`, `PRACTITIONER`, or `ADMIN`; returns `501` until storage and row-level access are implemented. |
| `POST /api/payments/checkout` | Requires authenticated `PATIENT` and same-origin browser request; returns `501` until verified payments are implemented. |
| `POST /api/stripe/webhook` | Requires Stripe signature header and server secret before implementation; does not use browser session auth. |
...
| `GET /api/reviews` | Public placeholder returning safe `501` until persisted reviews exist. |
...
`lib/security/rate-limit.ts` provides an in-memory limiter for MVP hardening. It is adequate for local/single-process deployments only. Production must replace it with Redis, Upstash, Cloudflare, or another shared store so limits work across server instances.
...
5. Replace in-memory rate limiting with a shared production limiter.
